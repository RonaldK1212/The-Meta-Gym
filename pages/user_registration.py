...
def draw_labels(x_pos, y_pos):
    first_name_label =  ttk.Label(root, text="First Name*",     font = ("Arial", 12), background="#ffc815").place(x = x_pos,y = y_pos, anchor = "e")
    last_name_label =   ttk.Label(root, text="Last Name*",      font = ("Arial", 12), background="#ffc815").place(x = x_pos,y = y_pos+30, anchor = "e")
    email_label =       ttk.Label(root, text="Email*",          font = ("Arial", 12), background="#ffc815").place(x = x_pos,y = y_pos+2*30, anchor = "e")
    phone_label =       ttk.Label(root, text="Phone Number",    font = ("Arial", 12), background="#ffc815").place(x = x_pos,y = y_pos+3*30, anchor = "e")
    sex_label =         ttk.Label(root, text="Sex*",            font = ("Arial", 12), background="#ffc815").place(x = x_pos,y = y_pos+4*30, anchor = "e")
    dob_label =         ttk.Label(root, text="Date of Birth*",  font = ("Arial", 12), background="#ffc815").place(x = x_pos,y = y_pos+5*30, anchor = "e")
    weight_label =      ttk.Label(root, text="Weight",          font = ("Arial", 12), background="#ffc815").place(x = x_pos,y = y_pos+6*30, anchor = "e")
...
def fetch_data():
    global first_name, last_name, email, phone, sex, dob, weight
...
def data_verification():
    global first_name, last_name, email, phone, sex, dob, weight
    fetch_data()
    if not (first_name == "" or last_name == "" or email == "" or sex == 0 or dob == ""):
        #DATA IS AVAILABLE, VALIDATE IT HERE
        try:
            weight = float(weight)
        except:
            messagebox.showinfo("Error", "Make sure you only use numbers [0-9] or a decimal points '.' in your Weight.")
    else:
        messagebox.showinfo("Error", "Fill all required data marked with *")
